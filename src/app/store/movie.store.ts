...
export const MovieStore = signalStore(
  ...
  withMethods((store, httpClient = inject(HttpClient)) => ({
    ...
    // deleteUom(data: any) {
    //   const index = store.unitOfMeasurement().findIndex(item => item.id === data.id);
    //   store.unitOfMeasurement().splice(index, 1);
    //   patchState(store, { unitOfMeasurement: [...store.unitOfMeasurement()] });
    // },

    // updateCurrency(data: any) {
    //   const index = store.currency().findIndex((item: any) => item.code === data.code);
    //   if (index === -1) {
    //     patchState(store, { currency: [...store.currency(), data] });
    //   } else {
    //     store.currency()[index].name = data.name;
    //     store.currency()[index].code = data.code;

    //     patchState(store, { currency: [...store.currency()] });
    //   }
    // },

}))
// withHooks({
//   onInit: function ({ loadAllData, loadAllItemData }) {
//     loadAllData();
//     loadAllItemData();
//   },
// }),
